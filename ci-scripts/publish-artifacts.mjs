...
function main() {
  ...
  templatesMetadata.templates = (templatesMetadata.templates || []).map(
    (template) => ({
      ...template,
      screenshot: `public/${template.slug}/screenshot.png`,
      files: (template.files || []).map(
        (filePath) => `public/${template.slug}/${filePath}`
      ),
      repositoryUrl: `https://github.com/extension-js/examples/tree/main/examples/${template.slug}`
    })
  )
