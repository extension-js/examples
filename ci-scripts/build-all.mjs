...
async function buildExample(slug, browser) {
  ...
  try {
    ...
    const success = await new Promise((resolve) => {
      ...
      child.on('error', (error) => {
        console.error(
          `►►► \n[ERROR] Error spawning build process for ${slug} [${browser}]:`,
          error.message
        )
        resolve(false)
      })
...
if (!fs.existsSync(nodeModulesPath)) {
  ...
  try {
    ...
    console.error(
      '►►► \n[ERROR] Error installing workspace dependencies:',
      error.message
    )
        ...
        try {
          ...
          if (attempt === maxRetries) {
            ...
            try {
              ...
              console.error(
                `►►► \n[ERROR] Error installing dependencies for ${slug} after ${maxRetries} attempts + fallback:`,
