...
- Name: Vue Content Example
- Summary: Shows a small overlay UI on web pages, with an options page for one saved setting.
...
Shows a small overlay UI on web pages, with an options page for one
saved setting.
...
- storage: keeps the one setting the options page writes so it
  survives closing the page and restarting the browser. Nothing
  leaves the browser.
...
- 1.0.0 (unreleased): initial version from the content-vue template.
