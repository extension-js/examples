...
// Plain Less keeps the class name it is written with, so the content script
// can name it as a string. The modules flavor of this template cannot.
const LEFT_CLASS = 'content_script_left'
...
export default function initial() {
  ...
  function render(position) {
    ...
    // the host carries `all: initial !important`, which a plain write loses
    // to. The Less rule clears the opposite anchor, because a fixed box with
    // a width honors one anchor only.
    contentDiv.classList.toggle(LEFT_CLASS, position === 'left')
  }
