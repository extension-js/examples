...
- Name: Preact Content Example
- Summary: Shows an overlay UI on web pages, with an options page that moves it from right to left.
...
Shows an overlay UI on web pages, with an options page that moves it
from right to left.
...
- storage: keeps the one setting from the options page so it survives
  closing the page and restarting the browser. The content script reads
  the same setting to decide which edge the overlay sits on. Nothing
  leaves the browser.
...
- 1.0.0 (unreleased): initial version from the content-preact template.
