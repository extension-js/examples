...
- Summary: Shows an overlay UI on web pages, with an options page for one setting.
...
Shows an overlay UI on web pages, with an options page for one setting.
...
- storage: keeps the one setting from the options page so it survives
  ...
  the same setting to decide whether to show the overlay. Nothing leaves
  the browser.
