...
Reads text out of an image with [tesseract-wasm](https://github.com/robertknight/tesseract-wasm). The OCR engine, WebAssembly binary, and English language data are bundled into the extension, so opening a new tab does not fetch them from the network.

## Try it locally

```bash
npx extension@latest create my-newtab-typescript-tesseract --template newtab-typescript-tesseract
cd my-newtab-typescript-tesseract
npm install
npm run dev
```
