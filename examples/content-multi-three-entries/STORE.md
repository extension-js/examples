...
- Name: JavaScript Content (Multiple Entries) Example
- Summary: Injects four small elements into web pages, with an options page that hides the first one.
...
- This template stores one setting, whether the first injected
  element is shown, in the browser's own sync storage. It collects
...
Injects four small elements into web pages, with an options page that hides the first one.
...
click Open options on the top-left element and untick the setting to
watch that element disappear. The build is bundled, so AMO
...
- 1.0.0 (unreleased): initial version from the content-multi-three-entries template.
