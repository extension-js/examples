...
- Name: Less Modules Content Example
- Summary: Injects a small styled badge into web pages, with an options page that turns it off.
...
- This template stores one setting, whether the badge is shown, in
...
Injects a small styled badge into web pages, with an options page that turns it off.
...
click Open options on the badge and untick the setting to watch the
badge disappear. The build is bundled, so AMO
...
- 1.0.0 (unreleased): initial version from the content-less-modules template.
