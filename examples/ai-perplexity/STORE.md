...
- Name: Perplexity Sidebar
- Summary: Adds a sidebar panel for chatting with Perplexity.
...
- The user's Perplexity API key is stored with the extension storage
  API on the device. It is sent only to the provider's own API and
  never to any other server.
- Prompts and page text the user submits are sent to the Perplexity
  API to generate Perplexity responses. Nothing is transmitted until
...
Adds a sidebar panel for chatting with Perplexity.
...
A reviewer needs their own Perplexity API key to exercise the chat;
...
- 1.0.0 (unreleased): initial version from the ai-perplexity template.
