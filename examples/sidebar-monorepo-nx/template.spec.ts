...
test.describe('Monorepo Structure', () => {
  ...
  test('nx.json declares a cached build target with dist outputs', () => {
    const nxPath = path.join(monorepoRoot, 'nx.json')
    expect(fs.existsSync(nxPath)).toBe(true)
    const nx = JSON.parse(fs.readFileSync(nxPath, 'utf8'))
    expect(nx.targetDefaults.build).toBeDefined()
    expect(nx.targetDefaults.build.cache).toBe(true)
    expect(nx.targetDefaults.build.outputs).toContain('{projectRoot}/dist')
    expect(nx.targetDefaults.dev.cache).toBe(false)
  })
})
...
test.describe('Monorepo Build Artifacts', () => {
  ...
  test('no root-level monorepo files leak into build output', () => {
    // Common gotcha: nx.json, pnpm-workspace.yaml, root package.json
...
runtimeTest(
  ...
  async ({page}) => {
    ...
    runtimeTest.expect(text).toContain('Nx Monorepo')
  }
)
