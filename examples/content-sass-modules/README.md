...
The options page runs on the same pipeline, from `src/options/styles.module.scss`, so it wears hashed class names too and its script assigns them. The edge the badge sits on is a modifier class, and the content script toggles the imported name rather than the name as authored, because the authored name never reaches the browser.
