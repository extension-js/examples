...
- Name: Claude (Anthropic) Sidebar
- Summary: Adds a sidebar panel for chatting with Claude.
...
- The user's Anthropic API key is stored with the extension storage
  API on the device. It is sent only to the provider's own API and
  never to any other server.
- Prompts and page text the user submits are sent to the Anthropic
  API to generate Claude responses. Nothing is transmitted until
...
Adds a sidebar panel for chatting with Claude.
...
A reviewer needs their own Anthropic API key to exercise the chat;
...
- 1.0.0 (unreleased): initial version from the ai-claude template.
